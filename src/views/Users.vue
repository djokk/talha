<template>
  <section class="users">
      ...
      <div class="card-custom">
        <div class="table-custom">
          <div class="table-custom__header">
            <p class="w-1">{{ $t('login') }}</p>
            <p class="w-2">{{ $t('password') }}</p>
            <p class="w-3">{{ $t('userName') }}</p>
            <p class="w-4"></p>
          </div>
          <div class="table-custom__body">
            <div v-for="item in info" :key="item.id" class="table-custom__body-item">
              <p class="w-1">{{ item.Login }}</p>
              <p class="w-2">{{ item.Password }}</p>
              <p class="w-3">{{ item.FullName }}</p>
              <div class="w-4 btns">
    ...
    <div v-if="modalDelete" class="modal-delete">
      <form @submit.prevent="delUser()" class="form" action="">
        <p>{{ $t('textDelete') }}</p>
        <div class="btns">
    ...
    <div v-if="modalAdd" class="modal-add">
      <form @submit.prevent="addUser()" class="form" action="">
        <div class="name">
          <input v-model="login" :class="$v.login.$error ? 'is-invalid': ''" class="input" type="text">
          <span>{{ $t('login') }}</span>
        </div>
        <div class="name">
          <input v-model="password" :class="$v.password.$error ? 'is-invalid': ''" class="input" type="text">
          <span>{{ $t('password') }}</span>
        </div>
        <div class="name">
          <input v-model="fullName" :class="$v.fullName.$error ? 'is-invalid': ''" class="input" type="text">
          <span>{{ $t('userName') }}</span>
        </div>
        <div class="btns">
          <button v-if="btnChange != true" class="btn btn-success" type="submit">{{ $t('add') }}</button>
          <button v-else-if="btnChange == true" @click.prevent="editUser()" class="btn btn-warning" type="button">{{ $t('change') }}</button>
...
export default {
  ...
  methods: {
    ...
    addUser() {
      this.$v.$touch()
      if (this.$v.login.$invalid || this.$v.password.$invalid || this.$v.fullName.$invalid) {
        this.$toast.open({
    ...
    delUser() {
      this.loading = true;
      axios.get(`${this.api}/usersDelete?Id=${this.userId}`)
        .then(() => {
